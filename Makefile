...
$(doc).pdf: $(doc).tex inc/style.tex inc/beamerthemeMalte.sty inc/beamercolorthemeMalte.sty content/*.tex images/*.tex
	TEXINPUTS="$(TEXINPUTS)" $(latex) $(doc).tex
	(egrep -q $(rerun) $(doc).log && TEXINPUTS="$(TEXINPUTS)" $(latex) $(doc).tex) || true
	(egrep -q $(rerun) $(doc).log && TEXINPUTS="$(TEXINPUTS)" $(latex) $(doc).tex) || true
	
$(doc)-script.pdf: $(doc)-script.tex inc/style.tex inc/beamerthemeMalte.sty inc/beamercolorthemeMalte.sty content/*.tex images/*.tex
